...
type config struct {
	...
	llmAdapter   provider.LLMAdapter
...
func (conf *config) Init(cb api.ConfigCallbackHandler) error {
	...
	if err := conf.initLLMAdapter(); err != nil {
		return err
	}
...
func (conf *config) initLLMAdapter() error {
	switch conf.LlmAdapter.Type {
	case "openai":
		conf.llmAdapter = &provider.OpenAIAdapter{}
		return nil
	default:
		return fmt.Errorf("invalid llm adapter type: %s", conf.LlmAdapter.Type)
	}
}
